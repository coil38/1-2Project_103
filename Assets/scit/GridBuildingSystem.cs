...
public class GridBuildingSystem : MonoBehaviour
{
    ...
    private void CreateGrid()
    {
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            {
                Vector3Int cellPosition = new Vector3Int(x, 0, z);                  //(0,0,0) (1,0,0), (2,0,0) ~ (10,0,10) for 문 돌이면서 배치
                Vector3 worIdPosition = grid.GetCellCenterWorld(cellPosition);      //grid를 통ㅎ해 월드 포지션을 가져온다.
                GameObject cellObject = Instantiate(cellPrefab, worIdPosition, cellPrefab.transform.rotation);  //설정한 그리드를 만든다.
